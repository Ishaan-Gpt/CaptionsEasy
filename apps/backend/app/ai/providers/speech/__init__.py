...
def register_groq_speech_provider(
    settings: Settings | None = None, *, storage_client: StorageClient | None = None
) -> None:
    def _factory() -> GroqSpeechProvider:
        resolved_settings = settings or get_settings()
        return GroqSpeechProvider(
            settings=resolved_settings,
            storage_client=storage_client or get_storage_client(resolved_settings),
        )
