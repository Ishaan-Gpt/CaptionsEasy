...
class GroqCaptionProvider(CaptionProvider):
    ...
    async def _call_groq(self, prompt: str) -> dict[str, Any]:
        ...
        for i, api_key in enumerate(keys_to_try):
            ...
            try:
                ...
                if i < len(keys_to_try) - 1:
                    print(f"[Caption Provider] Call to Groq with primary API key failed: {e}. Trying backup API key...")
                    continue
                else:
                    raise
            finally:
        ...
        raise RuntimeError(
            "No Groq API key is configured (GROQ_API_KEY is missing/empty) — cannot plan captions."
        )
